    ...
    <div class="col-xs-12">
        <div class="table-responsive">
			...
			<table class="table table-striped table-bordered table-hover">
                ...
                {
					?>
                    <tr id="row_<?= $row->id; ?>" <?php if($row->status==0) { ?>
						...
						<td>
							...
							if($row->type=="slider"){
								echo "Slider ".$row->category_id;
							}
							if($row->type=="itemcategory"){
								$category_id = $row->category_id;
								$row1 =  $this->db->query ("select * from tbl_item_category where status=1 and id='$category_id'")->row(); 
								echo $row1->name;
							}
							if($row->type=="divisioncategory"){
								$category_id = $row->category_id;
								$row1 =  $this->db->query ("select * from tbl_division_category where status=1 and id='$category_id'")->row(); 
								echo $row1->name;
							}
							?>
                        </td>
