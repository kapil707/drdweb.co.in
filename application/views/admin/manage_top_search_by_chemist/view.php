			...
			<table class="table table-striped table-bordered table-hover">
                ...
                foreach ($result as $row)
                {
                    ...
                    <tr id="row_<?= $row->id; ?>">
						...
						<td>
							<?php $user_altercode = $row->user_altercode; 
							$row1 = $this->db->query("select name from tbl_acm where slcd='CL' and altercode='$user_altercode' order by id asc")->row();
							echo $row1->name; ?>
							(<?php echo $user_altercode ?>)
							
							<?php if(!empty($row->salesman_id)) { 
								echo "by Salesman : ($row->salesman_id)";
							} ?>
                        </td>
						<td>
							<?php $item_code = $row->item_code; 
							$row1 = $this->db->query("select item_name from tbl_medicine where i_code='$item_code' order by id asc")->row();
							echo $row1->item_name; ?>
							(<?php echo $item_code ?>)							
                        </td>
						<td>
