			...
			<table class="table table-striped table-bordered table-hover">
                ...
                foreach ($result as $row)
                {
                    ...
                    <tr id="row_<?= $row->id; ?>">
						...
						<td>
                        	<?php
							$row2 = $this->db->query("select name from tbl_acm where altercode='$row->user_altercode'")->row();
							?>
							<?= $row2->name; ?> (<?= $row->user_altercode;?>)
                        </td>
						<td>
                        	<?php
							if($row->salesman_id!=""){
							$row2 = $this->db->query("select * from tbl_users where customer_code='$row->salesman_id'")->row();
							?>
