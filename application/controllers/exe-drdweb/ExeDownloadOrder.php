...
class ExeDownloadOrder extends CI_Controller
{
	...
	public function download_order_test($order_id)
	{
		...
		$date = date("Y-m-d");
		$download_time = date("YmdHi");
		$result = $this->db->query("SELECT tbl_cart_order_test.*,tbl_chemist.code,tbl_chemist.slcd FROM tbl_cart_order_test LEFT JOIN tbl_chemist ON tbl_cart_order_test.chemist_id = tbl_chemist.altercode WHERE tbl_cart_order_test.id='$order_id' and download_time<='$download_time'")->result();
		// Process each row from the result set
		foreach ($result as $row) {
	...
	public function update_order_gstvno_test($order_id,$gstvno,$insert_total_line,$download_total_line)
	{
		if(!empty($order_id) && !empty($gstvno))
		{
			$row = $this->db->query("SELECT items_total FROM `tbl_cart_order_test` WHERE `id`='$order_id'")->row();
			$total = $row->items_total;

			$this->db->query("update tbl_cart_order_test set gstvno='$gstvno',download_status=1,download_line='$insert_total_line' where id='$order_id'");
