...
class Manage_medicine extends CI_Controller {
	...
	public function view()
	{
		...
		if($_GET["search"]){
			...
			$result = $this->db->query("select * from $tbl where (item_name like '%$search%' or company_full_name like '%$search%') order by id desc")->result();
		}
		...
		if($_GET["search"]){
			$data["search"] = $search = $_GET["search"];
			$query = $this->db->query("select * from $tbl where (item_name like '%$search%' or company_full_name like '%$search%') order by id desc LIMIT $per_page,100");
		}
