...
class Manage_email_notification extends CI_Controller {
	var $Page_title = "Manage Email Notification";	
	var $Page_name  = "manage_email_notification";
	var $Page_view  = "manage_email_notification";
	var $Page_menu  = "manage_email_notification";
	var $page_controllers = "manage_email_notification";
	...
	public function add()
	{
		...
		if(isset($Submit))
		{
			...
			{ 
				...
				$query1 = $this->db->query("select * from tbl_acm where slcd='CL' $where order by id asc")->result();
	...
	public function view()
	{
		...
		if($_GET["name"])
		{
			$name = $_GET["name"];
			$data["search"] = $name;
			$result = $this->db->query("select * from $tbl where date='$vdt' and user_email_id='$name' order by id desc")->result();
		}
		...
		{
			$name = $_GET["name"];
			$query = $this->db->query("select * from $tbl where date='$vdt' and user_email_id='$name' order by id desc LIMIT $per_page,100");
		}
