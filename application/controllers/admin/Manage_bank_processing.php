...
class Manage_bank_processing extends CI_Controller {
	...
	public function view()
	{
		...
		$query = $this->BankModel->select_query("SELECT upi_no,process_name,date,process_value,find_chemist_id,find_invoice_chemist_id,done_status, GROUP_CONCAT(id SEPARATOR ', ') AS id,GROUP_CONCAT(amount SEPARATOR ', ') AS amount, GROUP_CONCAT(orderid SEPARATOR ', ') AS orderid, GROUP_CONCAT(type SEPARATOR ', ') AS type FROM `tbl_bank_processing` GROUP BY upi_no,process_name,date,process_value,find_chemist_id,find_invoice_chemist_id,done_status");
