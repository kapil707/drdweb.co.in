...
class Manage_user_chemist_request extends CI_Controller {
	...
	public function view()
	{
		...
		$query = $this->db->query("select tbl_chemist.id,tbl_chemist.code,tbl_chemist.altercode,tbl_chemist.name,tbl_chemist.email,tbl_chemist.mobile,tbl_chemist.status,tbl_chemist_other.exp_date,tbl_chemist_other.status as status1 from tbl_chemist,tbl_chemist_other,tbl_new_password where tbl_chemist.code=tbl_chemist_other.code and tbl_chemist.altercode=tbl_new_password.chemist_code order by tbl_chemist.id desc");
	...
	public function search_user($page_type)
	{
		...
		$query = $this->db->query("select tbl_chemist.id,tbl_chemist.address,tbl_chemist.address1,tbl_chemist.address2,tbl_chemist.address3,tbl_chemist.address3,tbl_chemist.code,tbl_chemist.altercode,tbl_chemist.name,tbl_chemist.email,tbl_chemist.mobile,tbl_chemist.status,tbl_chemist_other.exp_date,tbl_chemist_other.status as status1 from tbl_chemist,tbl_chemist_other where tbl_chemist.code=tbl_chemist_other.code and tbl_chemist.altercode='$search' order by tbl_chemist.id desc limit 10")->result();
...
EOD;
		...
		$query = $this->db->query("select tbl_chemist.id,tbl_chemist.address,tbl_chemist.address1,tbl_chemist.address2,tbl_chemist.address3,tbl_chemist.address3,tbl_chemist.code,tbl_chemist.altercode,tbl_chemist.name,tbl_chemist.email,tbl_chemist.mobile,tbl_chemist.status,tbl_chemist_other.exp_date,tbl_chemist_other.status as status1 from tbl_chemist,tbl_chemist_other where tbl_chemist.code=tbl_chemist_other.code and (tbl_chemist.altercode like '%$search%' or tbl_chemist.name like '%$search%') and tbl_chemist.id!='$id' order by tbl_chemist.id desc limit 10")->result();
		foreach($query as $row)
		{
			$altercode = $row->altercode;
			$name = $row->name;
...
{"items":[<?= $items;?>]}
	...
	public function password_create1(){
		...
		$row = $this->db->query("select tbl_chemist.code from tbl_chemist,tbl_chemist_other where tbl_chemist.code=tbl_chemist_other.code and tbl_chemist.id='$id' order by tbl_chemist.id desc")->row();
		$code = $row->code;
		$this->send_email_for_password_create($code,$password);
		$password = md5($password);
		$this->db->query("update tbl_chemist_other set password='$password' where code='$code'");
		echo "ok";
	}
