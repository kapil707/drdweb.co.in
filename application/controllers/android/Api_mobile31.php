...
class Api_mobile31 extends CI_Controller {
...
<?php
	...
	{
		header("Content-type: application/json; charset=utf-8");
		error_reporting(0);//http://drdistributor.com/android/api_mobile31/main_function/get?device_id=ok&user_type=chemist&chemist_id=v153&latitude=0&longitude=0&versioncode=24&firebase_token=ok&count_medicine=0&count_draft=0
		$items = "";
		if($page_type=="get")
		{
		...
		if($device_id!="")
		{
			...
			$where = array('user_altercode'=>$chemist_id,'user_type'=>$user_type,'device_id'=>$device_id,'latitude'=>$latitude,'longitude'=>$longitude,);
			$row = $this->Scheme_Model->select_row("tbl_acm_info",$where);
			if($row->id=="")
			{
				$dt = array(
				'latitude'=>$latitude,
				...
				$this->Scheme_Model->insert_fun("tbl_acm_info",$dt);
			}
			...
			$title2 = "New arrivals";
			$items2 = $this->Chemist_Model->new_medicine_this_month();
			$items2 = "[$items2]";
			
			/**********************must_buy_medicines_json************/
			$title3 = "Hot selling";
			$items3 = $this->Chemist_Model->hot_selling_today_json();
			$items3 = "[$items3]";
	...
	public function search_medicine($page_type)
	{
		...
		if($session_id!="")
		{
			...
			foreach($keyword1 as $row)
			{
				...
				$keyword_other1.="title like '%".$row."%' or ";
			}
			/*if ($keyword_other != ''){
				$keyword_other = substr($keyword_other, 0, -2);
			}*/
			$keyword = str_replace("%20"," ",$keyword);
			
			$items = $this->Chemist_Model->search_medicine($keyword,$keyword_other,$keyword_other1);
?>
[<?= $items;?>]
<?php
		}
