...
class BankWhatsAppModel extends CI_Model  
{
	...
	public function whatsapp_update_upi(){
		...
		if($working==0){
			...
			$result = $this->BankModel->select_query("SELECT p.upi_no, wm.id as whatsapp_id, wm.vision_text FROM tbl_bank_processing AS p JOIN tbl_whatsapp_message wm ON p.amount = wm.amount and wm.date BETWEEN DATE_SUB(p.date, INTERVAL 1 DAY) AND DATE_ADD(p.date, INTERVAL 1 DAY) AND (REPLACE(TRIM(wm.vision_text), ' ', '') LIKE CONCAT('%xx', RIGHT(TRIM(p.upi_no), 4), '%') or REPLACE(TRIM(wm.vision_text), ' ', '') LIKE CONCAT('%xx', RIGHT(TRIM(p.orderid), 4), '%')) and p.whatsapp_id=''");
			$result = $result->result();
			foreach($result as $row) {
				$working = 1;
	...
	public function whatsapp_update_reply_message(){
		...
		$result = $this->BankModel->select_query("SELECT reply.body AS reply_body1, wm.id as whatsapp_id FROM tbl_whatsapp_message AS wm LEFT JOIN tbl_bank_processing AS bp ON bp.whatsapp_id = wm.id LEFT JOIN tbl_whatsapp_message AS reply ON reply.reply_id = wm.message_id WHERE wm.reply_status=0 and reply.body!=''");
		$result = $result->result();
		foreach($result as $row) {
			if($row->reply_body){
